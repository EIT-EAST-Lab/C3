...
class LLMRayActorAsync(BaseLLMRayActor):
    ...
    async def update_weights_cuda_ipc(self, names, dtypes, shapes, ipc_handles_list, empty_cache_last: bool = True):
        """Update many weights through CUDA-IPC in a single Ray task."""
        if not names:
            return True
        assert len(names) == len(dtypes) == len(shapes) == len(ipc_handles_list), (
            "update_weights_cuda_ipc: length mismatch"
        )
        for i, (n, dt, sh, ipc) in enumerate(zip(names, dtypes, shapes, ipc_handles_list)):
            ...
            await self.update_weight_cuda_ipc(n, dt, sh, ipc, empty_cache=empty_cache)
        return True
