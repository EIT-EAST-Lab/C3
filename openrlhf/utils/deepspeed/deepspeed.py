...
class DeepspeedStrategy(ABC):
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        ...
        # --- HARD FIX: avoid huge temp allocation from DS grad prescale ---
        # Some DS versions do: return grad / gradient_accumulation_steps() (out-of-place)
        # inside _backward_prologue_per_tensor, which can allocate multi-GB tensors and OOM,
        # especially when vLLM holds large GPU memory.
        try:
            enable = str(os.environ.get("DS_INPLACE_GRAD_PRESCALE", "1")).strip().lower() not in {"0", "false", "no"}
            if enable and hasattr(engine, "_backward_prologue_per_tensor"):

                def _patched_backward_prologue_per_tensor(self_engine, grad):
                    if grad is None:
                        return grad
                    try:
                        gas = int(self_engine.gradient_accumulation_steps())
                    except Exception:
                        gas = 1
                    # Exact semantics: grad /= GAS, but do it in-place to avoid allocating a full copy.
                    if gas != 1 and torch.is_tensor(grad) and grad.is_floating_point():
                        try:
                            grad.div_(gas)
                            return grad
                        except Exception:
                            return grad / gas
                    # gas == 1 => returning grad unchanged is equivalent and avoids allocation too.
                    return grad

                engine._backward_prologue_per_tensor = types.MethodType(_patched_backward_prologue_per_tensor, engine)
                # DS backward_hook calls preprocess_per_tensor_fn; keep it consistent.
                engine.preprocess_per_tensor_fn = engine._backward_prologue_per_tensor
                logger.warning("DeepSpeed inplace grad prescale patch enabled (DS_INPLACE_GRAD_PRESCALE=1).")
        except Exception as e:
            logger.warning(f"DeepSpeed inplace grad prescale patch failed: {e}")
