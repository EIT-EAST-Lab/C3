...
# Expected benchmark suite per task: the suites of the main table, since the
# evaluation protocol of 2026-09-15 (docs/32_evaluation_protocol.md).
EXPECTED_DATASOURCES: Dict[TaskType, List[str]] = {
    "math": ["MATH500", "Minerva-Math", "AMC23", "AIME24", "AIME25"],
    "code": ["MBPP-test", "MBPP+"],
}

# The saturation controls. They are reported once, in the appendix, and are not
# part of the main table, so they are listed apart from EXPECTED_DATASOURCES and
# an aggregation that does not find them is not missing anything.
APPENDIX_DATASOURCES: Dict[TaskType, List[str]] = {
    "math": ["GSM8K-test", "CMATH-test"],
    "code": [],
}

# Reported columns that are more than one prepared file, the same merge
# scripts/70_rebuild/final_eval.py writes as its `AIME` entry: the two AIME
# suites are read as one 60-problem suite. Here the parts arrive as accuracies
# rather than as per-problem scores, so the merged value is the mean of the
# parts; that equals the mean over the 60 problems because AIME24 and AIME25
# carry 30 problems each. A part that is missing leaves the column empty rather
# than reporting half of it.
MERGED_TABLE_COLUMNS: Dict[str, Tuple[str, ...]] = {"AIME": ("AIME24", "AIME25")}

# Table column order (paper-facing order).
TABLE_COLUMNS: List[str] = ["MATH500", "Minerva-Math", "AMC23", "AIME", "MBPP+", "MBPP-test"]
APPENDIX_TABLE_COLUMNS: List[str] = ["GSM8K-test", "CMATH-test"]
...
def _merged_n_questions(parts: Sequence["DatasourceMetrics"]) -> int:
    """Problem count of a merged column: the sum, or -1 when a part has none.

    The metrics path records -1, which means "not counted here", so a sum that
    included it would be a smaller number pretending to be a count.
    """
    counts = [int(p.n_questions) for p in parts]
...
def _render_latex_table(*, summary: Mapping[str, Any], out_path: str, scale: float, digits: int) -> None:
    ...
    lines.append("% Appendix columns, reported apart from the main table: "
                 + " | ".join(APPENDIX_TABLE_COLUMNS))
...
def _aggregate(
    *,
    ...
    for run in runs:
        ...
        # The merged reported columns, per run, so the spread across seeds is the
        # spread of the merged number and not of its parts.
        for merged, parts in MERGED_TABLE_COLUMNS.items():
            if not all(part in exp_ds for part in parts):
                continue
            greedy_parts = [prof_data["greedy"].get(part) for part in parts]
            n10_parts = [prof_data["n10"].get(part) for part in parts]
            if any(p is None for p in greedy_parts) or any(p is None for p in n10_parts):
                continue
            ...
            raw_rows.append(
                {
                    ...
                    "seed": run.seed,
                    "datasource": merged,
                    "Greedy": _mean([p.greedy for p in greedy_parts]),
                    "P@1": _mean([p.pass1 for p in n10_parts]),
                    "P@10": _mean([p.pass10 for p in n10_parts]),
                    "n_questions_greedy": _merged_n_questions(greedy_parts),
                    "n_questions_n10": _merged_n_questions(n10_parts),
                    "merged_from": list(parts),
                }
            )
    ...
    raw_payload = {
        ...
        "contract": {
            ...
            "appendix_datasources": APPENDIX_DATASOURCES,
            "table_columns": TABLE_COLUMNS,
            "appendix_table_columns": APPENDIX_TABLE_COLUMNS,
            "merged_table_columns": {k: list(v) for k, v in MERGED_TABLE_COLUMNS.items()},
        },
    }
