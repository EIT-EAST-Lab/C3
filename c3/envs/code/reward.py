...
- C3_CODE_MAX_TIMEOUT_S:  ceiling on the wall clock a prepared row may ask for (default: 120)
"""
...
def _coerce_timeout(meta: Dict[str, Any], dataset_meta: Optional[Dict[str, Any]] = None, default: int = 15) -> int:
    ...
    needs: MBPP+ carries 120, which is twice what EvalPlus allows one of its tasks,
    because its tests are differential and run every input twice, once by the reference
    and once by the candidate. One MBPP+ problem needs it: its reference alone runs for
    about 30 seconds, both sides together measured 57.6, and the executor turns this
    value into an RLIMIT_CPU of timeout + 1 in the child, so the default of 15 and even
    EvalPlus's own 60 would score that problem 0 for every candidate. The larger of the
    two sources wins, so a row that needs 120 never silently gets 15, and the row's claim
    alone is bounded by C3_CODE_MAX_TIMEOUT_S (120) so that a data file cannot ask for an
    hour. A timeout the task file sets is the operator's own choice and is not bounded
    here.
    """
    seconds = int(default)
    ...
    if isinstance(dataset_meta, dict):
        ...
        if isinstance(declared, (int, float)) and declared > 0:
            cap = _env_int("C3_CODE_MAX_TIMEOUT_S", 120)
